...
import mlflow
import pandas as pd
from IncomeModel.data import get_data, clean_df, DIST_ARGS
from IncomeModel.encoders import TimeFeaturesEncoder, DistanceTransformer, AddGeohash, Direction, \
    DistanceToCenter
from IncomeModel.utils import compute_rmse, simple_time_tracker
...
#MLFLOW_URI = "https://mlflow.incomemodel.ai/"


class Trainer(object):
    ESTIMATOR = "Linear"
    EXPERIMENT_NAME = "IncomeModel"
...
if __name__ == "__main__":
    ...
    # Get and clean data
    experiment = "Federico_IncomeModel"
    if "Federico" in experiment:
    ...
    df = get_data(**params)
    df = clean_df(df)
    y_train = df["ingreso_neto_comprobado"]
    X_train = df.drop("ingreso_neto_comprobado", axis=1)
    del df
