...
from income.metrics import RMSLE, MAE, MAPE
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import seaborn as sns
...
def evaluate_model(df=None, pred="validated_final_model",true="ingreso_neto_comprobado",metric="mape", plot_totals=False):
    df["ratio_income_min_w"] = df[true]/4000
    
    df["mape"] = df.apply(lambda x: MAPE(x[true], x[pred]), axis=1)
